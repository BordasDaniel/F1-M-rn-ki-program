...
def summarize_stints(data):
    ...
    # Stintváltás: ha az átlagos kopás csökken (reset), vagy keverékváltás
    stint_indices = [0]
    ...
    stint_indices.append(len(data))  # záró index
...
def suggest_race_strategy(
    data, 
    race_length_laps=70, 
    required_pitstops=1, 
    wear_threshold=70, 
    min_compound_changes=2
):
    ...
    # Hány kört bír egy keverék a degradáció alapján?
    stint_lengths = {}
    ...
    # Sorrend: Hard → Medium → Soft (ha van mindhárom)
    ...
    # Válasszunk min_compound_changes különböző keveréket (legtartósabbakat)
    ...
    stints = []
    # Először minden kötelező keveréket használunk egyszer
    ...
    # Ha maradt még stint, folytassuk a leghosszabb bírhatót, de ne ismételjünk egymás után
    last_comp = stints[-1][0] if stints else None
    for i in range(stints_left):
        # Válasszunk olyat, ami nem az előző
    ...
    # Ha maradt kör, adjuk hozzá az utolsó stinthoz
...
def get_race_stints(data, race_length_laps=70, required_pitstops=1, wear_threshold=70, min_compound_changes=2):
    degradation = calculate_compound_degradation(data)
    if not degradation:
        print("Not enough data for race strategy.")
        return []
...
def plot_strategy_with_paging(data, stints):
    # Három oldal: 0 = szabadedzés, 1 = kerékcserék, 2 = stratégia
    current_page = [0]
    total_pages = 3

    fig, ax = plt.subplots(figsize=(14, 5))

    # Előfeldolgozás: compound színek, kerékcserék detektálása
    compound_column = [col for col in data.columns if 'Compound' in col][0]
    colors = {'Soft': 'red', 'Medium': 'orange', 'Hard': 'gray'}
    ...
    # Kerékcsere/stintváltás detektálás
    tire_columns = ['Front left tire usage (percentage)',
                    'Front right tire usage (percentage)',
                    'Rear left tire usage (percentage)',
                    'Rear right tire usage (percentage)']
    ...
    def show_page(page):
        ax.clear()
        if page == 0:
            # 1. oldal: részletes szabadedzés köridők
            ...
            # 2. oldal: kerékcserék/stintváltások
...
def main():
    ...
    # Stratégia és vizualizáció lapozhatóan
